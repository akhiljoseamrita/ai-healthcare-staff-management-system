...
def staff_recommendations_for_job(request):
    ...
    if job_id:
        ...
        return JsonResponse({"job_id": job.id, "results": score_for_job(job)})
        ...
        grouped_results.append(
            {
                "department": department.name,
                "job_id": job.id,
                "results": score_for_job(job),
            }
        )

    return JsonResponse({"hospital_id": hospital_id, "results": grouped_results})
